...
- [ ] **Both operational deployment profiles pass isolated regression suites in CI.** The default/offline profile passes 201 tests, but the explicit hosted profile currently fails 21 tests because shared fixtures are unscoped and CI runs only one ambient/default profile. Add tenant-scoped hosted fixtures, pin profile settings per test run, and run an offline/hosted CI matrix before treating Phase 10.2 as release-complete. *(verified 2026-06-07 12:42 UTC; see `docs/audits/2026-06-07-phase-10-implementation-verification.md`)*
