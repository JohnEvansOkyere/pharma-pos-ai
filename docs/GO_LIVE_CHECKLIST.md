...
- [/] **Both operational deployment profiles pass isolated regression suites in CI.** Hosted fixtures now create consistent organization/branch ownership across users, products, and batches; tests ignore ambient `.env` mode settings; customer-retention tests use real FK-backed tenants; and CI has an offline/hosted PostgreSQL matrix. Local CI-parity runs pass **204 tests in each profile**. Remaining evidence: the pushed GitHub Actions matrix must complete successfully. *(updated 2026-06-07 13:18 UTC; see `docs/audits/2026-06-07-phase-10-implementation-verification.md`)*
