...
- [x] **Globally stable tenant / deployment / device identifiers.** Control-plane UUIDs now identify organizations, branches, deployments, and devices across databases; event uploads include the complete UUID envelope and a deterministic deployment-scoped `aggregate_uid`. Central ingestion stamps tenant scope from the authenticated device registration rather than trusting local integer IDs. Existing rows receive repeatable UUIDv5 backfills; tenant-database creation/seeding remains in the separate provisioning item below. ✅ *(2026-06-07 08:18 UTC)*
- [x] **Tenant provisioning tooling:** `backend/scripts/provision_tenant.py` now allocates resumable global identity, creates an isolated paid Render Postgres instance or accepts an explicitly supplied isolated database, runs Alembic, seeds the matching tenant/branch/device and scoped admin, registers the same identity and token hash in the control plane, creates the dedicated Render backend, and records private resumable state. Render database access is restricted to one provisioner IP and disabled after completion. ✅ *(2026-06-07 08:37 UTC)*
- [x] **Per-tenant secrets management:** provisioning now obtains a unique database credential, generates unique application/admin/publish secrets, requires owner-only provider-specific SMS credentials for hosted tenants, rejects reserved variables and detected cross-tenant sensitive-key reuse, stores plaintext only under ignored `0600` state, and injects the approved bundle directly into the dedicated Render backend. Optional tenant AI/SMTP/Telegram credentials use the same controlled path. ✅ *(2026-06-07 09:02 UTC)*
- [x] **Both operational deployment profiles pass isolated regression suites in CI.** Hosted fixtures create consistent organization/branch ownership across users, products, and batches; tests ignore ambient `.env` mode settings; customer-retention tests use real FK-backed tenants; and CI runs the complete PostgreSQL suite in both offline and hosted profiles. Local CI-parity runs passed **204 tests in each profile**, and GitHub Actions run [`27093777473`](https://github.com/JohnEvansOkyere/pharma-pos-ai/actions/runs/27093777473) completed both backend matrix jobs successfully for commit `33892d5`. ✅ *(2026-06-07 13:27 UTC; see `docs/audits/2026-06-07-phase-10-implementation-verification.md`)*
