...
A branch-assigned admin or manager remains branch-restricted; role permissions do not silently expand data scope. An organization-level admin is represented by `organization_id` with `branch_id = NULL` and may work across that organization's branches. In `online_pos`, an organization-unscoped user is rejected from operational queries instead of receiving unfiltered data.
