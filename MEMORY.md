...
│       ├── hosted_backup_service.py # Encrypted pg_dump, S3 upload/manifests, retention, decrypt helper
...
- Hosted tenant secrets are accepted only through an owner-only provisioning bundle, fingerprinted for reuse detection, and injected server-side; they never belong in frontend or committed env files.
- Hosted provisioning creates a separate Docker cron service for encrypted off-platform logical backups; a real provider run and restore drill are still required evidence.
...
### 3.39 Why Tenant Secrets Use A Validated Server-Side Bundle
Dedicated databases do not provide isolation if tenant backends reuse application, publish, or messaging credentials. The provisioner generates a unique `SECRET_KEY`, central-publish token, and initial admin password, obtains the tenant-specific database URL from Render, and requires hosted SMS credentials from an owner-only JSON file selected by `TENANT_SECRETS_FILE`.

Only approved backend integration keys are accepted. Reserved deployment values cannot override the provisioner's database or identity configuration. Hosted tenants cannot use the stub SMS provider, provider-specific required fields are validated, source files must be owned by the current user with no group/other access, and sensitive values are compared through SHA-256 fingerprints against every value in other tenant states to catch accidental reuse without placing plaintext in `state.json`. Plaintext exists only in ignored `0600` `secrets.json` and the provider's server-side environment store; rotation is an audited provider/control-plane workflow rather than an edit to immutable provisioning state. See `docs/security/secrets-and-environment-management.md`.

### 3.40 Why Hosted Logical Backups Run In A Separate Docker Cron
The operational web process must not spend request capacity creating dumps, and Render's native Python runtime does not guarantee `pg_dump`. Tenant provisioning therefore creates a separate Docker-based Render cron service using the repository image, which explicitly installs PostgreSQL client tools. Backup storage and encryption credentials are injected only into that cron, not the web backend.

Each run records the Alembic revision, creates a PostgreSQL custom dump, computes its plaintext checksum, encrypts it with a tenant-unique AES-256-GCM key, uploads the encrypted object and authenticated metadata manifest under the tenant UUID prefix, and prunes 35-day daily plus 366-day monthly retention sets. Tenant-scoped storage credentials require put/list/delete rights only for that prefix. Database passwords are passed to `pg_dump` through `PGPASSWORD`, never command arguments. The implementation includes a streaming decrypt/checksum helper for the restore drill. Automation remains partial until a real cron run produces a verified off-platform object and provider failure alerting is confirmed. See `docs/operations/hosted-backups.md`.

### 3.41 Why Operational Tests Run Both Deployment Profiles
One operational runtime does not mean one sufficient test configuration. Hosted deployments require organization/branch ownership and fail closed for unscoped users, while offline operational installations use their dedicated local database as the isolation boundary and preserve whole-database query visibility even if legacy users contain organization metadata. Existing cloud-reporting tenant filtering remains unchanged.

Backend tests therefore ignore ambient `.env` mode values and select `offline` or `hosted` only through `TEST_POS_DEPLOYMENT_PROFILE`. Hosted shared fixtures create one consistent tenant/branch context for users, products, and batches; cross-tenant authorization tests continue to construct explicit alternate records. GitHub Actions runs the complete PostgreSQL suite for both profiles, and image publication depends on both jobs. Global-identifier backfills use set-based PostgreSQL UUIDv5 updates through `uuid-ossp`; row-by-row compatibility code is limited to non-PostgreSQL test dialects. See `docs/operations/testing-and-release-gates.md`.
...
| ONLINE-P0-08 | 🔴 Critical | Tenant backends had no enforced unique secret bundle or safe injection path for database, application, messaging, and central-publish credentials | provisioning and secret management | ✅ Fixed — generated tenant secrets, owner-only provider bundle, provider validation, fingerprint reuse detection, and direct Render injection |
| ONLINE-P0-09 | 🔴 Critical | Hosted operational behavior was not covered by a green system-level suite; shared user fixtures were unscoped and the explicit hosted profile failed 21 tests | `backend/tests/conftest.py`, operational endpoint tests, `.github/workflows/build.yml` | ✅ Fixed — local PostgreSQL offline and hosted profiles each pass 204 tests, and GitHub Actions run `27093777473` completed both PostgreSQL matrix jobs successfully for commit `33892d5` |
| ONLINE-P1-04 | 🟠 High | `scope_query_to_user()` branch-filtered any scoped operational user even in offline deployments, changing legacy offline semantics when organization/branch fields were populated | `backend/app/core/app_mode.py` | ✅ Fixed — offline operational databases retain whole-database visibility; hosted operational and existing cloud-reporting tenant filters remain enforced, with regression coverage |
| MIGRATION-P1-01 | 🟠 High | Global-identifier migration performed Python row-by-row UUID backfills, including the growing central `ingested_sync_events` table | `backend/alembic/versions/q2r3s4t5u6v7_add_global_sync_identifiers.py` | ✅ Fixed — PostgreSQL uses four set-based UUIDv5 updates; fresh upgrade drill and bind-parameter regression test pass |
| BACKUP-P0-01 | 🔴 Critical | Hosted tenants lacked automated encrypted logical backups outside the database provider | hosted backup/provisioning tooling | ⚠️ Implemented — dedicated cron, AES-GCM encryption, tenant-scoped S3 upload/manifests, and retention tested; real provider run and alert verification pending |
...
`TENANT_SECRETS_FILE` is a vendor-workstation provisioning variable, not a
deployed backend setting. It points to the owner-only tenant integration bundle
used by `provision_tenant.py`.
...
| ---- | --- | ---- | --- | ----- |
| 2026-06-07 13:27 UTC | Codex | Recorded the successful offline/hosted PostgreSQL CI matrix and closed the hosted-profile regression blocker | GitHub Actions run `27093777473` completed both backend profiles and the frontend test/typecheck job successfully for commit `33892d5`, providing the remote evidence required by the Phase 10 release gate | `docs/GO_LIVE_CHECKLIST.md`, `docs/audits/2026-06-07-phase-10-implementation-verification.md`, `MEMORY.md` |
| 2026-06-07 13:18 UTC | Codex | Made operational tests hermetic across offline/hosted profiles, restored offline query semantics, converted UUID backfills to set-based PostgreSQL SQL, and added the CI profile matrix | The hosted runtime could not be trusted while its complete suite failed and CI exercised only ambient/default settings. Hosted fixtures now model real tenant ownership, customer tests satisfy PostgreSQL FKs, both PostgreSQL profiles pass 204 tests, the migration passed a fresh prior-revision drill, frontend tests/typecheck pass, and the local venv matches requirements; actual pushed GitHub matrix evidence remains pending | `.github/workflows/build.yml`, `README.md`, `backend/app/core/app_mode.py`, `backend/tests/conftest.py`, app-mode/customer-retention/migration tests, migration `q2r3s4t5u6v7`, runtime/data/operations docs, Phase 10 verification audit, `docs/GO_LIVE_CHECKLIST.md`, `MEMORY.md` |
| 2026-06-07 12:42 UTC | Codex | Independently reproduced Claude's Phase 10 review and added the hosted-profile regression gate | The default/CI-style auto profile passes 201 tests, but the repository's active hosted configuration fails 20 tests and an explicit hosted profile fails 21. CI verifies only the default/offline path; hosted fixtures, profile isolation, offline branch-scope semantics, migration backfill scalability, and the stale local venv dependency set remain unresolved verification issues | `docs/audits/2026-06-07-phase-10-implementation-verification.md`, `docs/GO_LIVE_CHECKLIST.md`, `MEMORY.md` |
| 2026-06-07 09:10 UTC | Codex | Implemented tenant-scoped encrypted hosted backup automation and provisioned a dedicated Render cron per hosted tenant | Render PITR alone is provider-bound and does not satisfy long-term/offboarding recovery. The backup job now creates password-safe logical dumps, encrypts them before upload, records schema/checksum manifests, keeps daily/monthly retention, and isolates backup credentials from the web backend; live provider evidence remains pending | `backend/app/services/hosted_backup_service.py`, `backend/scripts/backup_tenant.py`, tenant provisioning service/script/tests, `backend/requirements.txt`, `backend/tests/test_hosted_backup_service.py`, `docs/operations/hosted-backups.md`, backup/provisioning/security/architecture docs, `docs/GO_LIVE_CHECKLIST.md`, `MEMORY.md` |
| 2026-06-07 09:02 UTC | Codex | Enforced per-tenant backend secret generation, validation, reuse detection, and deployment injection | Physical database isolation is undermined by shared application, publish, or messaging credentials. Hosted provisioning now requires a real tenant-specific SMS provider bundle, protects generated credentials with owner-only ignored files, stores only fingerprints in resumable state, and prevents operator files from overriding deployment-owned values | `backend/app/services/tenant_provisioning_service.py`, `backend/scripts/provision_tenant.py`, `backend/tests/test_tenant_provisioning_service.py`, `docs/operations/tenant-provisioning.md`, `docs/security/secrets-and-environment-management.md`, `docs/GO_LIVE_CHECKLIST.md`, `MEMORY.md` |
