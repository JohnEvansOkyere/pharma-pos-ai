...
### 3.30 Why Two Deployment Tiers From One Codebase (Online-First + Offline-First)
Market feedback confirmed two distinct customer profiles: city pharmacies with stable internet want online-first features (customer registration, digital receipts, health follow-ups, real-time AI), while village pharmacies with unreliable or no internet need a fully offline local installation. Rather than maintaining two separate products, the system supports both through `APP_MODE` tiers: `local_pos` (offline-first, local PostgreSQL, optional sync), `online_pos` (online-first, Supabase as primary DB, full POS writes to cloud, customer retention module enabled, offline fallback queue for network drops), and `cloud_reporting` (vendor dashboard, reports only). Village clients continue unchanged. City clients get a direct cloud database connection with no sync outbox needed — the AI sees live data, receipts can be sent immediately after sale, and health follow-ups are scheduled in real time. The offline fallback queue for city clients is a short-lived local cache (IndexedDB) that auto-flushes on reconnect, not the full outbox/projection architecture used by village clients. See `docs/DUAL_MODE_DEPLOYMENT_PLAN.md` for the full implementation roadmap and `docs/ONLINE_FIRST_ARCHITECTURE.md` for the complete technical reference (file map, API, tenant isolation, offline queue internals, deployment guide).
...
| ONLINE-P0-01 | 🔴 Critical | `online_pos` has direct object-access tenant bypasses: sale creation, sale reversal/read, product get/update/delete/batch/receive-stock, stock adjustments, and dashboard queries load or aggregate rows without enforcing `current_user.organization_id` ownership | `sales.py`, `products.py`, `stock_adjustments.py`, `dashboard.py` | ⚠️ Accepted — must fix before any online POS rollout |
| ONLINE-P0-02 | 🔴 Critical | Online user administration is not tenant-scoped; org admins can list/update/delete users across tenants, and created users are not stamped with organization/branch IDs | `users.py`, `models/user.py` | ⚠️ Accepted — must fix before any online POS rollout |
| ONLINE-P0-03 | 🔴 Critical | Tenant IDs are stamped on some parent rows but not consistently on `SaleItem`, `ProductBatch`, or `InventoryMovement`, weakening audit/reporting isolation in `online_pos` | `sales.py`, `products.py`, `inventory_service.py`, `models/sale.py`, `models/product.py`, `models/inventory_movement.py` | ⚠️ Accepted — must fix before any online POS rollout |
| ONLINE-P0-04 | 🔴 Critical | Sale creation accepts arbitrary `customer_id` and does not verify the customer belongs to the current user's organization before linking receipt/follow-up workflows | `sales.py`, `customers.py` | ⚠️ Accepted — must fix before any online POS rollout |
