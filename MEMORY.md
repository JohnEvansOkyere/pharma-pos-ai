...
| ONLINE-P0-09 | 🔴 Critical | Hosted operational behavior is not covered by a green system-level suite; shared user fixtures are unscoped and the explicit hosted profile fails 21 tests | `backend/tests/conftest.py`, operational endpoint tests, `.github/workflows/build.yml` | ⚠️ Open — add hosted tenant fixtures, isolate settings per profile, and run offline/hosted CI matrix entries |
| ONLINE-P1-04 | 🟠 High | `scope_query_to_user()` branch-filters any scoped operational user even in offline deployments, changing legacy offline semantics when organization/branch fields are populated | `backend/app/core/app_mode.py` | ⚠️ Open — explicitly decide whether offline installs are branch-restricted and add regression coverage |
| MIGRATION-P1-01 | 🟠 High | Global-identifier migration performs Python row-by-row UUID backfills, including the growing central `ingested_sync_events` table | `backend/alembic/versions/q2r3s4t5u6v7_add_global_sync_identifiers.py` | ⚠️ Open — replace with set-based PostgreSQL updates before central data volume grows |
