...
The previous `online_pos` design used one shared Supabase operational database, skipped the outbox, and relied on application-level tenant filtering. That design is superseded by Decision 3.33. `online_pos` may remain temporarily as a runtime/feature label while the migration is implemented, but customer-retention features must be decoupled from shared-database tenancy assumptions. The eventual modes should describe deployment capability, not two incompatible tenant-isolation models. See `docs/DUAL_MODE_DEPLOYMENT_PLAN.md` and `docs/ONLINE_FIRST_ARCHITECTURE.md`; both documents require revision before being treated as current implementation guidance.

### 3.31 Why Docker Mode Switching Uses Application-Scoped Environment Profiles
The source-built Docker stack keeps private backend profiles at `backend/.env_local` and `backend/.env_cloud`, copied into `backend/.env`, and browser-safe frontend profiles at `frontend/.env_local` and `frontend/.env_cloud`, copied into `frontend/.env.local`. The local pair selects `local_pos` with Docker PostgreSQL; the cloud pair selects `online_pos` and a managed PostgreSQL `DATABASE_URL` while the app containers still run locally. Docker Compose no longer hardcodes backend local mode. The frontend must be rebuilt with `--env-file frontend/.env.local` because Vite mode values are compile-time settings. Packaged client installs remain unchanged and continue to use `backend/.env` through `docker-compose.client.yml`.

### 3.32 Why The Transitional Online POS UI Uses One Dashboard Plus An AI Manager Workspace
This decision describes the temporary `online_pos` runtime while it is migrated to the isolated per-tenant architecture in Decision 3.33. It does not preserve the shared-database design. The operational Dashboard is the tenant's single source for live sales, stock, margin, and risk. A compact AI briefing belongs on that Dashboard, while chat, persistent findings, recommendations, and weekly reports belong in a dedicated **AI Manager** workspace. The existing Cloud Dashboard remains specific to `cloud_reporting`, where projected branch data, sync health, reconciliation, and multi-branch owner reporting are valid concerns.

The AI API remains server-side under `/api/ai-manager`; it is not embedded as frontend-only logic. Its reporting data source must be mode-aware: `online_pos` reads tenant-scoped operational `Sale`, `SaleItem`, `Product`, `ProductBatch`, and inventory movement tables directly, while `cloud_reporting` reads the approved `Cloud*` projection tables. Navigation must not be consolidated until the online operational dashboard is tenant-scoped and this live-data AI adapter exists.
...
| ONLINE-P0-01 | 🔴 Critical | The current shared-database `online_pos` runtime has direct object-access tenant bypasses across sale, product, stock, and dashboard paths | `sales.py`, `products.py`, `stock_adjustments.py`, `dashboard.py` | ⚠️ Superseded architecture — do not onboard another tenant to the shared DB; database-per-organization removes cross-organization exposure, but branch ownership checks remain required |
| ONLINE-P0-02 | 🔴 Critical | Current shared-database online user administration is not tenant-scoped | `users.py`, `models/user.py` | ⚠️ Superseded architecture — database-per-organization removes cross-organization user access; branch and admin-role boundaries still require tests |
| ONLINE-P0-03 | 🔴 Critical | Tenant IDs are stamped on some parent rows but not consistently on `ProductBatch` and every inventory path, weakening audit/reporting isolation in `online_pos`; sale creation now stamps `Sale`, `SaleItem`, `InventoryMovement`, and audit rows before commit | `sales.py`, `products.py`, `inventory_service.py`, `models/sale.py`, `models/product.py`, `models/inventory_movement.py` | ⚠️ Partial — sale path fixed; remaining inventory paths must be completed before rollout |
...
| ONLINE-P1-02 | 🟠 High | Frontend expects `/api/auth/heartbeat`, but no backend route exists; current check treats 404 as online instead of requiring a 2xx response | `useOnlineStatus.ts`, `auth.py` | ⚠️ Accepted |
...
| `APP_MODE`                  | `local_pos` for village/local POS, `online_pos` for city/cloud POS, or `cloud_reporting` for the vendor portal | ✅       |
