...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE dosageform AS ENUM ('TABLET', 'CAPSULE', 'SYRUP', 'INJECTION', 'SUSPENSION', 'CREAM', 'OINTMENT', 'DROPS', 'POWDER', 'INHALER', 'SUPPOSITORY', 'PATCH', 'OTHER')")
    op.execute("CREATE TYPE prescriptionstatus AS ENUM ('PRESCRIPTION_REQUIRED', 'PRESCRIPTION_OPTIONAL', 'OTC')")
    op.execute("CREATE TYPE salestatus AS ENUM ('COMPLETED', 'PENDING', 'CANCELLED', 'REFUNDED')")
